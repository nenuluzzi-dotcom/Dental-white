    ...
    try:
        with pdfplumber.open(io.BytesIO(contenido)) as pdf:
            ...
            for idx, pagina in enumerate(pdf.pages):
                ...
                # Convertir solo esta página
                kw = {"first_page": idx+1, "last_page": idx+1, "dpi": 200}
                if POPPLER_PATH:
                    kw["poppler_path"] = POPPLER_PATH
                imgs   = convert_from_bytes(contenido, **kw)
                img_pil = imgs[0]
                ...
                for i, f in enumerate(anclas_izq):
                    ...
                    # N° afiliado
                    cta_n = "S/D"
                    m = re.search(r'N[°º.\s]\s*(\d+)', txt_der)
                    ...
                    if "AFILIADO" in txt_der.upper():
                        try:
                            ...
                            for linea in lineas[:2]:
                                ...
                                if limpia and not any(kw in limpia for kw in
                                        ['DIRECCION','PROVINCIA','COBRO','PLAN',
                                         'MONTO','CTA','TALON','RECUERDE']):
                                    parts.append(limpia)
                                else:
                                    break
                    ...
                    # Recorte imagen → Supabase Storage
                    ...
                    if recorte.size > 0:
                        ...
                        _, buf = cv2.imencode('.jpg', recorte)
                        try:
                            supabase.storage.from_("cupones")\
                                .upload(nombre_img, buf.tobytes(),
                                        {"content-type": "image/jpeg"})
                            img_url = supabase.storage.from_("cupones")\
                                .get_public_url(nombre_img)
                        except: pass
        ...
        return {"ok": True, "detectados": detectados, "paginas": total}
